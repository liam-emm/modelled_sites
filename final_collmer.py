# -*- coding: utf-8 -*-
"""
Created on Mon Sep 11 17:31:47 2023
...
scollmer1 = xr.open_dataset('scollmer1.grib', engine = 'cfgrib', chunks = 1000)
scollmer2 = xr.open_dataset('scollmer1.grib', engine = 'cfgrib', chunks = 1000)
scollmer3 = xr.open_dataset('scollmer3.grib', engine = 'cfgrib', chunks = 1000)

collmer1 = xr.open_dataset('collmer1.grib', engine = 'cfgrib', chunks = 1000)
collmer2 = xr.open_dataset('collmer2.grib', engine = 'cfgrib', chunks = 1000)
collmer3 = xr.open_dataset('collmer3.grib', engine = 'cfgrib', chunks = 1000)
collmer4 = xr.open_dataset('collmer4.grib', engine = 'cfgrib', chunks = 1000)
collmer5 = xr.open_dataset('collmer5.grib', engine = 'cfgrib', chunks = 1000)
collmer6 = xr.open_dataset('collmer6.grib', engine = 'cfgrib', chunks = 1000)
collmer7 = xr.open_dataset('collmer7.grib', engine = 'cfgrib', chunks = 1000)
collmer8 = xr.open_dataset('collmer8.grib', engine = 'cfgrib', chunks = 1000)
collmer9 = xr.open_dataset('collmer9.grib', engine = 'cfgrib', chunks = 1000)


#%% Creating numpy arrays of relevent parameters
collmer_u1 = collmer1.u10.values[:,0,0]
collmer_u2 = collmer2.u10.values[:,0,0]
collmer_u3 = collmer3.u10.values[:,0,0]
collmer_u4 = collmer4.u10.values[:,0,0]
collmer_u5 = collmer5.u10.values[:,0,0]
collmer_u6 = collmer6.u10.values[:,0,0]
collmer_u7 = collmer7.u10.values[:,0,0]
collmer_u8 = collmer8.u10.values[:,0,0]
collmer_u9 = collmer9.u10.values[:,0,0]

collmer_v1 = collmer1.v10.values[:,0,0]
collmer_v2 = collmer2.v10.values[:,0,0]
collmer_v3 = collmer3.v10.values[:,0,0]
collmer_v4 = collmer4.v10.values[:,0,0]
collmer_v5 = collmer5.v10.values[:,0,0]
collmer_v6 = collmer6.v10.values[:,0,0]
collmer_v7 = collmer7.v10.values[:,0,0]
collmer_v8 = collmer8.v10.values[:,0,0]
collmer_v9 = collmer9.v10.values[:,0,0]

collmer_t1 = collmer1.t2m.values[:,0,0]
collmer_t2 = collmer2.t2m.values[:,0,0]
collmer_t3 = collmer3.t2m.values[:,0,0]
collmer_t4 = collmer4.t2m.values[:,0,0]
collmer_t5 = collmer5.t2m.values[:,0,0]
collmer_t6 = collmer6.t2m.values[:,0,0]
collmer_t7 = collmer7.t2m.values[:,0,0]
collmer_t8 = collmer8.t2m.values[:,0,0]
collmer_t9 = collmer9.t2m.values[:,0,0]

nans_collmer_s1 = scollmer1.ssr.values
nans_collmer_s2 = scollmer2.ssr.values
nans_collmer_s3 = scollmer3.ssr.values

collmer_s1 = nans_collmer_s1[~np.isnan(nans_collmer_s1)]
collmer_s2 = nans_collmer_s2[~np.isnan(nans_collmer_s2)]
collmer_s3 = nans_collmer_s3[~np.isnan(nans_collmer_s3)]

u = np.concatenate([collmer_u1, collmer_u2, collmer_u3, collmer_u4, collmer_u5, collmer_u6, collmer_u7,
                    collmer_u8, collmer_u9])

v = np.concatenate([collmer_v1, collmer_v2, collmer_v3, collmer_v4, collmer_v5, collmer_v6, collmer_v7,
                    collmer_v8, collmer_v9])

t = np.concatenate([collmer_t1, collmer_t2, collmer_t3, collmer_t4, collmer_t5, collmer_t6, collmer_t7,
                    collmer_t8, collmer_t9]) - 273.15

s = np.concatenate([collmer_s1, collmer_s2, collmer_s3]) / 3600
...
Australian = pytz.timezone('Australia/Perth')
latitude = -31.54
longitude = 118.24

# Wind

wind_capacity = 222      # MW
hub_height = 80          # m
P_rated_wind = 2         # MW
v_rated = 13.5           # m/s
v_cut_in = 4             # m/s
...
strings_per_inverter = 268
number_of_inverters = 44

location = Location(latitude = latitude, longitude = longitude, tz = 'Australia/Perth',
                    altitude = 328, name = 'Collgar Wind and Merredin Solar')
...
ax.set_title('Modelled Power Curve for Goldwind GW140 3.57 MW')
ax.legend()

plt.savefig('Collgar Turbine Power Curve', dpi = 600)
...
times = pd.date_range(start = '1990-01-01 08:00:00', end = '2023-01-01 07:00:00',
...
summer = (weather_data['ghi'].loc['2017-01-01']).values
autumn = (weather_data['ghi'].loc['2019-04-05']).values
winter = (weather_data['ghi'].loc['2005-07-31']).values
spring = (weather_data['ghi'].loc['2003-10-25']).values
...
spring_shift = midday_index - spring_avg_idx - 1
...
plt.title('Average Monthly Solar Generation at Merredin Solar Farm for varying La Nina Conditions')
plt.xticks(index + bar_width/2, month_names, rotation=45)
plt.legend(loc='lower right')
plt.tight_layout()
plt.savefig('Monthly Average Solar Generation (Merredin Solar Farm)', dpi=600)
plt.show()
...
plt.title('Average Monthly Wind Generation at Collgar Wind Farm for varying La Nina Conditions')

plt.xticks(index + bar_width/2, month_names, rotation=45)
plt.legend(loc='lower right')
plt.tight_layout()
plt.savefig('Monthly Average Wind Generation (Collgar Wind Farm).png', dpi=600)
plt.show()
...
plt.title('SOI vs Wind PC1 (Collgar Wind Farm)')
plt.savefig('Collgar SOI PCA 1 (Wind).png', dpi=600)
plt.show()
...
# ax2.legend()
plt.text(0.05, 0.95, f'Correlation: {correlation_coefficient:.3f}', transform=ax1.transAxes, fontsize=12, verticalalignment='top')
plt.title('ONI vs Wind PC1 (Collgar Wind Farm)')
plt.savefig('Collgar ONI PCA 2 (Wind).png', dpi=600)
plt.show()
...
# ax2.legend()
plt.text(0.05, 0.95, f'Correlation: {correlation_coefficient:.3f}', transform=ax1.transAxes, fontsize=12, verticalalignment='top')
plt.title('SOI vs PC1 (Merredin Solar Farm)')
plt.savefig('Merredin SOI PCA 1 (Solar).png', dpi=600)
plt.show()
...
# ax2.legend()
plt.text(0.05, 0.95, f'Correlation: {correlation_coefficient:.3f}', transform=ax1.transAxes, fontsize=12, verticalalignment='top')
plt.title('ONI ANOM vs PC2 (Merredin Solar Farm)')
plt.savefig('Merredin ONI PCA 2 (Solar).png', dpi=600)
plt.show()
